  ...
  return (
    <Card className="border-brand/40 p-4" aria-label={`Ride offer, ${secondsLeft} seconds to answer`}>
      <div className="flex items-center justify-between">
        <span className="text-xs font-medium text-brand-strong">New ride · {humanize(offer.vehicleCategory)}</span>
        <span className="flex items-center gap-1 text-sm font-semibold tabular-nums text-fg" aria-live="off">
      ...
      <p className="mt-3 flex items-center gap-2 text-sm text-fg">
        <Navigation className="size-4 text-brand-strong" aria-hidden /> {formatDistance(offer.distanceToPickupMeters)} to pickup
      </p>
