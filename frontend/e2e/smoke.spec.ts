...
import { capture, clickMap, DEMO, demoPassword, PUBLIC_SERVICES_TIMEOUT_MS, signIn } from "./support";
...
test("a passenger books a ride that a simulated driver completes, then sees the trip", async ({ page }) => {
  ...
  await capture(page, "passenger-booking");
  await request.click();
  ...
  await capture(page, "passenger-trip-details", { fullPage: true });
});
...
test("an admin sees the platform's rides and system state", async ({ page }) => {
  ...
  await capture(page, "admin-system");
});
