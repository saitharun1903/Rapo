...
| GET | `/rides/{id}` | Ride detail: its passenger, its assigned driver, a driver holding a pending unexpired offer for it, or an admin; anyone else `404 RIDE_NOT_FOUND`. A driver who holds only an offer gets `passenger: null`: who booked is shown once they accept |
...
| GET | `/rides/{id}/messages` | In-ride chat, oldest first: `[{id, rideId, senderId, senderRole: PASSENGER\|DRIVER, body, sentAt}]`. The passenger and the assigned driver only (`404` for anyone else, a driver with only an offer included). After a re-dispatch the new driver sees only messages sent since they accepted |
| POST | `/rides/{id}/messages` | `{body (1-500, trimmed)}` → `201` message. Only while a driver is engaged (DRIVER_ASSIGNED to IN_PROGRESS), else `409 CHAT_CLOSED`. Rate limit `CHAT` (20 per minute per user). Pushed to both participants on `/user/queue/ride-messages`; messages are deleted 30 days after the ride ends |
