...
| 13 CI/CD | ✅ Verified | Commit 9b0dd54 ran all six workflows on the upgraded actions: `secret-scan` 36133040128 (gitleaks over all history), `backend-ci` 36133040180, `frontend-ci` 36133040067, `load-test` 36133040109 and `cache-benchmark` 36133040089 passed. Its `e2e` run 36133040139 failed one Playwright test (see Found); `e2e` 36133960150 on 032f888, whose only change was a diagnostic step, passed all 5 tests and the dashboard check. Image builds: 158 s with an empty layer cache (36133040139), 15 s with a warm one (36133960150). **Not yet verified:** publishing to GHCR, which runs only on `main` |
| 14 Deployment | 🟡 Prepared, not deployed | CI: `backend-ci` 36137763323 on 00320df (ManagedKafkaIT: the SASL/SCRAM settings against Redpanda; HealthProbesIT), `e2e` 36137763277, `secret-scan` 36137763381, `frontend-ci` 36137239101. `free-tier-fit` 36137239023: the backend at 512 MiB and 0.1 CPU started in 180 s, peaked at 441 MiB, completed 16 rides with no failures; login p50 8.5 s ([deployment.md](deployment.md#measured)). **Not yet done:** the accounts (Neon, Upstash, Redpanda, Render, Vercel) are the owner's to create; then the public URL is checked end to end |
...
**Phase 13 delivered:**
- **Secret scanning:** gitleaks, checksum-verified, over the whole history on every push and pull request; findings become annotations that name file, line and rule but never the value. Before it was enabled, a local scan of all 46 commits found one match, the fixed password of test-only accounts, now accepted by fingerprint in `.gitleaksignore` so a change to that line is scanned again.
- **Cached, published images:** the e2e job builds the three images with Buildx bake from the compose file, caching layers per image in GitHub's cache. On `main`, the images that just passed Playwright and the dashboard check are pushed to GHCR with the commit SHA and `latest`; nothing is rebuilt for publishing.
- **Current actions and dependencies:** checkout 7, setup-node 7, setup-java 6 and upload-artifact 7 (release notes read for breaking changes first), ending the Node 20 deprecation warning. Dependabot opens weekly grouped update pull requests for Maven, npm, Actions, the Dockerfiles and the compose images.
- **Diagnostics:** a failed Playwright test now also annotates the page snapshot at the failure.
- **Docs:** architecture section 21 shows the pipeline as built; development.md has a CI section.

**Design changes made in Phase 13:** no formatter gate (the architecture listed a Spotless check that was never built; adding one now would reformat every file at once). Publishing sits in the e2e job, after the tests, rather than in a separate job, so the published images are the ones tested.

**Found while verifying Phase 13:**
- **A flaky end-to-end test:** in e2e run 36133040139 the "books from their own location" test never got a fare to request (30 s), while the public OSRM server returned responses the backend could not read and the other four tests passed; the application code was the same as in the previous green runs. It passed on the next run without a change. The cause is not confirmed, which is why the page-snapshot annotation was added.

**Carried forward:**
- **On merge to main:** check that the e2e run publishes the three images to GHCR. Done: e2e run 36135187626 on 9de3543 published all three, and each manifest is served to an anonymous client (HTTP 200), so a host can pull them without credentials.

**Phase 14 delivered so far:**
- **Managed Kafka:** SASL_SSL with SCRAM from four environment variables (plaintext by default), and a configurable partition count. ManagedKafkaIT runs them against Redpanda with authorization on: all 32 topics created with one partition, a round trip, a wrong password refused. TLS itself is exercised only by the deployment.
- **Probes on the public port:** `/livez` and `/readyz`, status only; metrics and full health stay on the management port.
- **Render Blueprint** (`render.yaml`): the tested GHCR image, free plan, Singapore, secrets prompted rather than stored, JVM options for 512 MB. On main, the e2e job deploys the image it published, by commit tag, once the deploy hook secret exists.
- **free-tier-fit workflow:** the backend at the free plan's size, with the Blueprint's settings.
- **Vercel guard:** a hosted build fails without an https backend URL and a wss WebSocket URL.
- **docs/deployment.md:** the free tiers as checked (with sources), why Kafka is a 30-day Redpanda trial (Aiven's free tier holds 5 topics; RideFlow has 32), the setup, rollback, and the trade-offs.
