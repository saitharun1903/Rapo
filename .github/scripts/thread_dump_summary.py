...
Reports the threads that matter for a stall: those running RideFlow code and those BLOCKED on a monitor, each
with its state and top frames, so an intermittent hang can be diagnosed from the run page (and the checks
...
def main() -> int:
    ...
    suspects = [thread for thread in all_threads
                if thread[1] == "BLOCKED" or any(APP_PACKAGE in frame for frame in thread[2])]
    print(f"::warning title=Thread dump::{len(all_threads)} threads, {len(suspects)} running RideFlow code "
          f"or blocked; the first {min(len(suspects), MAX_THREADS)} follow.")
